...
PRE_MATCH_MINS   = int(os.environ.get("PRE_MATCH_MINS", "5"))  # minutos de antecedência
COMPETITION_ID   = "WC"
...
def process_match(match):
    ...
    # ── Alerta pré-jogo (5 min antes) ──────────────────────
    if status == "SCHEDULED" and not prev["notified_pre"]:
        kickoff_str = match.get("utcDate")  # ex: "2026-06-15T21:00:00Z"
        if kickoff_str:
            kickoff = datetime.fromisoformat(kickoff_str.replace("Z", "+00:00"))
            now     = datetime.now(timezone.utc)
            diff    = (kickoff - now).total_seconds() / 60  # minutos até o jogo
    ...
    if status == "IN_PLAY" and not prev["notified_start"]:
        ...
        prev["notified_pre"]   = True  # garante que não manda o pré depois
